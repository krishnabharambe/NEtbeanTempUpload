...
//                tbpw1.addCell(new Paragraph("Total GST :" + pdfGeneration.EnglishNumberTranslator.convert(dFormat.format(data44.getDouble("totalgst"))), H10));
    ...
    public GSTINV(String Saleid) {
        try {
            final String Destination = getpath(Saleid);
            File file = new File(Destination);
            file.getParentFile().mkdirs();
            createPdf(Destination, Saleid);
    ...
    private String getpath(String varcaseid) {
        ...
        return desktopPath.replace("\\", "/") + "/CometREP/report/GSTInvoice/Invoice " + varcaseid + " " + datenew + " " + bpath1 + ".pdf";
    }
    ...
    private void createPdf(String destination, String Saleid) throws FileNotFoundException, DocumentException {
        try {
             ...
             * adding header
             */
            PdfPCell tb1cell1 = new PdfPCell(new Paragraph("GST INVOICE", H12B));
...
//            //bill to 
            ...
            ResultSet data222 = db.getdata("SELECT name as name,address as address,Contact as contact,Email as Email,GSTIN as GSTIN,pantan as pantan from Customer INEER JOIN Sales ON id = Sales.CustomerId WHERE SaleId =  '" + Saleid + "'");
            if (!data222.getString("Name").equals("")) {
            ...
            if (!data222.getString("Email").equals("")) {
            ...
            ResultSet data = db.getdata("Select * from Sales Where Saleid = '" + Saleid + "'");
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy");
            String nowyear = LocalDate.now().format(formatter);
            invoiceinfo.addCell(new Paragraph("Invoice Id : RHE-"+Saleid, H10));
            invoiceinfo.addCell(new Paragraph("Invoice Date : " + data.getString("saledate").replace("-", "/"), H10));
            data.close();
            int paymentchecker = 0;
            ResultSet data1 = db.getdata("select count(*) as checker FROM SalePayment WHERE saleid = '" + Saleid + "'");
            paymentchecker = data1.getInt("checker");
            data1.close();
            if (paymentchecker > 0) {
                String aboutpayment, expecteddate, status;
                ResultSet data2 = db.getdata("Select * from SalePayment WHERE saleid = '" + Saleid + "'");
            ...
            ResultSet data12 = db.getdata("select count(*) as checker FROM SaleDelivery WHERE saleid = '" + Saleid + "'");
            deliverychecker = data12.getInt("checker");
            data12.close();
            if (deliverychecker > 0) {
                String aboutpayment, expecteddate, status;
                ResultSet data2 = db.getdata("Select * from SaleDelivery WHERE saleid = '" + Saleid + "'");
...
//            data tags
            ...
            ResultSet data121 = db.getdata("SELECT StockTitle,stockHSN as HSN,stockprice as price,discount,stockgst,quantity,total FROM myTranscation WHERE invoiceid = '" + Saleid + "'");
            ...
            ResultSet data45 = db.getdata("SELECT stockHSN as HSN,sum(TOTAL) as taxable,stockgst as gst,(stockgst/2) as subgstpercentage,SUM((TOTAL/100)*STOCKGST) as totalgst,SUM((TOTAL/100)*STOCKGST)/2 as halfgst FROM myTranscation WHERE invoiceid = '" + Saleid + "'");
...
//            ResultSet data43 = db.getdata("SELECT HSN,sum(((myTranscation.price-(myTranscation.price/100)*discount)*quantity)) as taxable,gst,(gst/2) as subgstpercentage,sum(((((myTranscation.price-(myTranscation.price/100)*discount)/100)*gst)*quantity)) as totalgst,sum(((((myTranscation.price-(myTranscation.price/100)*discount)/100)*gst)*quantity)/2) as halfgst FROM myTranscation INNER JOIN Stock ON myTranscation.StockId = Stock.StockId WHERE myTranscation.SaleId = '" + Saleid + "' GROUP BY HSN");
            ResultSet data43 = db.getdata("SELECT stockHSN as HSN,sum(TOTAL) as taxable,stockgst as gst,(stockgst/2) as subgstpercentage,SUM((TOTAL/100)*STOCKGST) as totalgst,SUM((TOTAL/100)*STOCKGST)/2 as halfgst FROM myTranscation WHERE invoiceid = '" + Saleid + "' GROUP BY stockhsn");
...
//            ResultSet data44 = db.getdata("SELECT HSN,sum(((myTranscation.price-(myTranscation.price/100)*discount)*quantity)) as taxable,gst,(gst/2) as subgstpercentage,sum(((((myTranscation.price-(myTranscation.price/100)*discount)/100)*gst)*quantity)) as totalgst,sum(((((myTranscation.price-(myTranscation.price/100)*discount)/100)*gst)*quantity)/2) as halfgst FROM myTranscation INNER JOIN Stock ON myTranscation.StockId = Stock.StockId WHERE myTranscation.SaleId = '" + Saleid + "'");
            ResultSet data44 = db.getdata("SELECT stockHSN as HSN,sum(TOTAL) as taxable,stockgst as gst,(stockgst/2) as subgstpercentage,SUM((TOTAL/100)*STOCKGST) as totalgst,SUM((TOTAL/100)*STOCKGST)/2 as halfgst FROM myTranscation WHERE invoiceid = '" + Saleid + "'");
...
//            tbpw1.addCell(new Paragraph("Total GST : " + pdfGeneration.EnglishNumberTranslator.convert(dFormat.format(data44.getDouble("totalgst"))), H10));
            ...
            PdfPCell celltc;
            ResultSet data3 = db.getdata("SELECT * FROM Invoiceterms ORDER BY id");
            while (data3.next()) {
